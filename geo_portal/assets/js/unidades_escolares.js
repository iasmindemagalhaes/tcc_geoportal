...
$.ajax('http://localhost:8080/geoserver/ows?',{
  ...
  data: {
    ...
    typename: 'bdgeo:escola_distrito_sede', //nome da camada no Geoserver
    srsname: 'EPSG:4326',
...
function handleJson(data) {
    selectedArea = L.geoJson(data, {
      ...
      onEachFeature: function(feature, layer) {
        layer.bindTooltip(`<strong>${feature.properties.name}</strong>`+`</br>`+`${feature.properties.bairro}`,{
          direction: 'top',
          permanent: false,
          sticky: true,
          offset: [10, 0],
...
var wfsLayerdist = new L.featureGroup();
...
$.ajax('http://localhost:8080/geoserver/ows?',{
  ...
  data: {
    ...
    version: '1.0.0', //versão dada pelo Geoserver
    request: 'GetFeature', //Retorna uma seleção de recursos de uma fonte de dados, incluindo geometria e valores de atributo
    typename: 'bdgeo:escola_rural', //nome da camada no Geoserver
    srsname: 'EPSG:4326',
    outputFormat: 'text/javascript', //formato de saída
    },
  dataType: 'jsonp',
...
var map = L.map("map", {
    ...
    zoom: 5,
    minZoom: 3,
    layers: [osm, wmsLayer, wfsLayerdist, wfsLayer, wmsLayer2]
})
...
var overlayMaps = {
    "Bairros": wmsLayer,
    "Unidades de Educação":wfsLayerdist,
